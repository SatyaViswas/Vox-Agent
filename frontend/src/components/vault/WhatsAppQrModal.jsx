      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          ...
          if (poll.status === "linked") {
            ...
            stopPolling();
            setStatus("error");
            setError(poll.error || "The QR session expired. Try linking again.");
          }
        } catch (err) {
          stopPolling();
          setStatus("error");
          setError(err.message || "Lost connection while waiting for the scan.");
        }
      }, POLL_MS);
    } catch (err) {
      setStatus("error");
      setError(err.message || "Failed to start a WhatsApp Web session.");
    }
  }, [userId, stopPolling]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative glass-panel w-full max-w-sm p-6 flex flex-col items-center gap-4">
        <div className="flex items-center justify-between w-full">
          <h2 className="font-semibold">Link WhatsApp Web</h2>
          <button
            onClick={handleClose}
        ...
        {status === "linked" ? (
          <div className="flex flex-col items-center gap-3 py-6">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 text-center">
              WhatsApp Web is linked. VoxAgent can now drive it for your agents.
            </p>
          </div>
        ) : status === "error" ? (
          ...
          <div className="flex flex-col items-center gap-3 py-10">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Opening a real WhatsApp Web session…</p>
          </div>
        ) : (
          <>
            <div className="w-44 h-44 rounded-lg bg-white flex items-center justify-center overflow-hidden">
              {qrImage ? (
                <img src={qrImage} alt="WhatsApp Web QR code" className="w-full h-full object-contain" />
              ) : (
