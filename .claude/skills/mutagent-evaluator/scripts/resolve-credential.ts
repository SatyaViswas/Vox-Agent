 ...
 *
 * A `creds_ref` / `credential_ref` in `.mutagent/config.yaml` is NEVER a raw secret
 * — it names where the value lives. This turns the ref into the value by a fixed
 * PRECEDENCE:
 *
 ...
 *
 * Raw secret VALUES never enter config.yaml. Pure core with INJECTED deps (env map
 * + file reader) → deterministic tests. Secret-safety: this module returns the
 * value only to its caller; it never logs it.
 */
