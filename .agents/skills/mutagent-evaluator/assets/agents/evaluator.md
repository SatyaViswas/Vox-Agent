...
commands:
  - discover:
      ...
      suite_awareness:
        ...
        - "TOMBSTONES (G5): the brief's `tombstones[]` lists criteria the OPERATOR REJECTED — each with the statement verbatim and, when given, WHY. Do NOT re-propose a rule the operator already declined: that is the rediscovery loop this list exists to stop, and re-asking the same question every run is how a review surface loses their attention. Read the rationale: 'not measurable' means re-propose only if you can state it as a genuinely binary check; 'out of scope for this subject' means do not re-propose it at all. If the traces show the rejected behaviour again with MATERIALLY STRONGER evidence than the rejection was made on, you MAY re-propose it — say explicitly that it was previously rejected and what is new. INFORMS, NEVER BLOCKS, exactly like `criteria[]`."
        - "ABSENT/EMPTY brief ⇒ proceed exactly as before (a first run has no suite; that is normal, not a gap). The same holds per-key: no `tombstones[]` ⇒ nothing was ever rejected."
      # G5 · THE IDENTITY CALL — the one semantic comparison only an agent can make.
      # A lexical score cannot see that "the agent announced an action and never performed
      # it" and "reply asserts a completed unsubscribe with no corresponding tool call" are
      # ONE rule: they share almost no vocabulary. scripts/criterion-identity.ts frames the
      # call (builds the shortlist, validates your answer, applies the veto guards, records
      # the outcome) — it never makes the judgement, and you never write to any store.
      criterion_identity:
        - "WHEN ASKED: you receive ONE proposal statement + a SHORTLIST of things the operator already ruled on (each with its id, statement and whether it was accepted or rejected). Answer ONLY about that pair-set."
        - "ANSWER SHAPE: {relation, matchedId, rationale}. relation ∈ same | narrower | broader | different | unsure. `matchedId` is REQUIRED for same/narrower/broader and MUST be an id from the shortlist you were given — an id from anywhere else is discarded and the proposal is shown as new."
        - "`unsure` IS A FIRST-CLASS ANSWER, not a failure. A judge forced to choose is a judge that guesses, and a guess here silently suppresses a real finding. If the wording could plausibly be either, say unsure — the cost is one question to the operator; the cost of a wrong `same` is a criterion nobody ever sees again."
        - "JUDGE THE RULE, NOT THE WORDING: two statements are `same` when a trace failing one would fail the other, for the same reason. They are `narrower`/`broader` when one is a strict special case of the other (e.g. 'no card numbers in replies' vs 'no card numbers anywhere') — that is an AMENDMENT to the existing criterion, not a new rule and not a duplicate."
        - "DIFFERENT MEANS DIFFERENT: opposite polarity (includes vs excludes), opposite ordering (before vs after), opposite quantifier (at least vs at most) or a swapped subject (billing vs technical) are DIFFERENT rules no matter how similar the sentences look. Deterministic guards independently veto a `same` on these shapes, so claiming one is simply overruled — but do not rely on that: it is a backstop, not your job outsourced."
        - "YOU ARE NOT THE RULING AUTHORITY: you say whether it is the same rule. Whether it is then suppressed, amended or shown is decided by code from the operator's PRIOR ruling. Never argue for or against a criterion's merit here."
      compresses:
...
4. **LOCALIZE (node 8) + CRYSTALLIZE (node 9), then Write** `<trajectory_key>.verdict.json` — a
   ...
   **The dense map is MANDATORY** even when you skip the heavy walk: every matrix `criterionId` MUST
   ...
   `judgeSteps`/`agentSteps` walk to NON-PASS trajectories, but ALWAYS emit the dense map.
   **If you write BOTH surfaces for a criterion, they MUST agree** (G3): `verdicts[]` is your
   REASONING, `denseMap` is your COVERAGE — the dense map is not a second opinion and must never
   state a different result from your own detailed verdict for the same `criterionId`. A mismatch is
   REJECTED fail-loud at aggregate (`assertVerdictConsistency`) rather than silently resolved, because
   a judge contradicting itself would otherwise produce a confident scorecard with no sign of trouble.
   `na` is exempt — it is a coverage state (the criterion's precondition is falsified here), not a
   verdict. The
